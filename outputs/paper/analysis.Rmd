...
kable(UofT_fake, caption="Sample Dataset Rows") %>% 
...
kable(UofT_title %>% rename(JobTitle = outer), caption= "Three Most Frequent Job Titles by Year") %>% 
...
kable(UofT_prof_fivenum %>% rename(title = outer),caption = "Professor Salaries by Year, 2019") %>% 
  kableExtra::kable_styling(latex_options = c("HOLD_position"))
```
*Table 3*
...
kable(na_gender, caption= "Gender Encoding Iterations") %>% 
  kableExtra::kable_styling(latex_options = c("HOLD_position"))
```
*Table 4*
...
kable(UofT %>% group_by(`Calendar Year`,) %>% 
      ...
      caption="Employees by Year, with Mean Salaries") %>% 
    kableExtra::kable_styling(latex_options = c("HOLD_position"))
```
*Table 5*
...
kable(UofT_gen %>% replace_na(list(gender = 'unknown')) %>% group_by(gender,) %>% 
            summarize(count = n(), 
                      ...
                      max = max(SalaryPaid)), caption="Salaries by Gender (all years, all positions)") %>% 
    kableExtra::kable_styling(latex_options = c("HOLD_position"))
```
*Table 6* 
...
kable(UofT_gen %>% 
  ...
  mutate(pct_change = (count/lead(count) - 1) * 100) %>% 
...
```{r summary_model1}
kable(summary(fit1), caption="Model 1, Iteration 1") %>% 
  kableExtra::kable_styling(latex_options = c("stripped","scale_down", "HOLD_position"))
```
*Table 8*
...
```{r summary_model2}
kable(summary(fit2), caption="Model 1, Iteration 2") %>% 
  kableExtra::kable_styling(latex_options = c("stripped","scale_down", "HOLD_position"))
```
*Table 9*
...
```{r summary_model3}
kable(summary(fit3), caption="Model 1, Iteration 3") %>% 
  kableExtra::kable_styling(latex_options = c("stripped","scale_down", "HOLD_position"))
```
*Table 10*
...
kable(comp <- loo_compare(loo1, loo2, loo3), caption="Model 1, LOO analysis of all three iterations") %>% 
  kableExtra::kable_styling(latex_options = c("stripped","scale_down", "HOLD_position"))
```
*Table 11*
...
```{r summary_model4}
kable(summary(fit4), caption="Model 2, Iteration 1") %>% 
    kableExtra::kable_styling(latex_options = c("stripped","scale_down", "HOLD_position"))
```
*Table 12*
...
```{r summary_model5}
kable(summary(fit5), caption="Model 2, Iteration 2") %>% 
    kableExtra::kable_styling(latex_options = c("stripped","scale_down", "HOLD_position"))
```
*Table 13*
...
```{r summary_model6}
kable(summary(fit6), caption="Model 2, Iteration 3") %>% 
    kableExtra::kable_styling(latex_options = c("stripped","scale_down", "HOLD_position"))
```
*Table 14*
...
kable(comp <- loo_compare(loo4, loo5, loo6), caption="Model 2, LOO analysis of all three iterations") %>% 
    kableExtra::kable_styling(latex_options = c("stripped","scale_down", "HOLD_position"))
```
*Table 15*
